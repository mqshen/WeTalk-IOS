...
struct SWWebImageDownloaderOptions : RawOptionSetType {
    ...
    static var allZeros: SWWebImageDownloaderOptions { return self(rawValue: 0) }
    static var None: SWWebImageDownloaderOptions          { return self(rawValue: 0) }
    static var LowPriority: SWWebImageDownloaderOptions   { return self(rawValue: 1 << 0) }
    static var ProgressiveDownload: SWWebImageDownloaderOptions  { return self(rawValue: 1 << 1) }
    static var UseNSURLCache: SWWebImageDownloaderOptions   { return self(rawValue: 1 << 2) }
    static var IgnoreCachedResponse: SWWebImageDownloaderOptions   { return self(rawValue: 1 << 3) }
    static var ContinueInBackground: SWWebImageDownloaderOptions   { return self(rawValue: 1 << 4) }
    static var HandleCookies: SWWebImageDownloaderOptions   { return self(rawValue: 1 << 5) }
    static var AllowInvalidSSLCertificates: SWWebImageDownloaderOptions   { return self(rawValue: 1 << 6) }
    static var HighPriority: SWWebImageDownloaderOptions   { return self(rawValue: 1 << 7) }
}
...
class SWWebImageDownloader
{
    ...
    func downloadImage(url: NSURL, options: SWWebImageDownloaderOptions, progressHandler: SWWebImageDownloaderProgressHandler?,
        ...
        self.addProgressCallback(progressHandler, completeHandler: completeHandler, url: url) { () -> Void in
            ...
            let policy = options.rawValue & SWWebImageDownloaderOptions.UseNSURLCache.rawValue != 0 ? NSURLRequestCachePolicy.UseProtocolCachePolicy :
                NSURLRequestCachePolicy.ReloadIgnoringLocalAndRemoteCacheData
            let request = NSMutableURLRequest(URL: url, cachePolicy: policy, timeoutInterval: self.downloadTimeout)
            request.HTTPShouldHandleCookies = options.rawValue & SWWebImageDownloaderOptions.HandleCookies.rawValue != 0
            ...
            if options.rawValue & SWWebImageDownloaderOptions.HighPriority.rawValue != 0 {
                operation.queuePriority = NSOperationQueuePriority.High
            }
            else if options.rawValue & SWWebImageDownloaderOptions.LowPriority.rawValue != 0 {
