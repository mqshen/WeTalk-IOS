                ...
                if let completeHandler = self.completeHandler? {
                    ...
                    if (width + height == 0) {
                        if let properties = CGImageSourceCopyPropertiesAtIndex(imageSource, 0, nil)? {
                            ...
                            let property = properties as NSDictionary
                            if let val: AnyObject = property[kCGImagePropertyPixelHeight as String]? {
                                CFNumberGetValue(val as CFNumber, CFNumberType.LongType, &height)
                            }
                            if let val: AnyObject = property[kCGImagePropertyPixelWidth as String]? {
                                CFNumberGetValue(val as CFNumber, CFNumberType.LongType, &width)
                            }
                            if let val: AnyObject = property[kCGImagePropertyOrientation as String]? {
                    ...
                    if (width + height > 0 && totalSize < self.expectedSize) {
                        if var partialImageRef = CGImageSourceCreateImageAtIndex(imageSource, 0, nil)? {
                            ...
                            let temp: UInt = 8
                            let temp2: UInt = 4
                            if let bmContext = CGBitmapContextCreate(nil, width, height, temp, width * temp2, colorSpace, CGBitmapInfo(rawValue: CGBitmapInfo.ByteOrderDefault.rawValue | CGImageAlphaInfo.PremultipliedFirst.rawValue))? {
