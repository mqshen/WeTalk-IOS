...
func decodedImageWithImage(image: UIImage) -> UIImage {
    ...
    let infoMask: CGImageAlphaInfo = CGImageAlphaInfo(rawValue: bitmapInfo.rawValue & CGBitmapInfo.AlphaInfoMask.rawValue)!
    ...
    if (infoMask == CGImageAlphaInfo.None && CGColorSpaceGetNumberOfComponents(colorSpace) > 1) {
        ...
        bitmapInfo = CGBitmapInfo(bitmapInfo.rawValue & ~CGBitmapInfo.AlphaInfoMask.rawValue | CGImageAlphaInfo.NoneSkipFirst.rawValue)
    }
    else if (!anyNonAlpha && CGColorSpaceGetNumberOfComponents(colorSpace) == 3) {
        bitmapInfo = CGBitmapInfo(bitmapInfo.rawValue & ~CGBitmapInfo.AlphaInfoMask.rawValue | CGImageAlphaInfo.PremultipliedFirst.rawValue)
    }
...
func imageOrientationFromImageData(imageData: NSData) -> UIImageOrientation {
    ...
    if let imageSource = CGImageSourceCreateWithData(imageData, nil)? {
        if let properties = CGImageSourceCopyPropertiesAtIndex(imageSource, 0, nil)? {
            var prop = properties as NSDictionary
            //let prop = properties.__conversion()
            var exifOrientation = 0
            if let value: AnyObject = prop[kCGImagePropertyOrientation as String]? {
...
func frameDuration(index: UInt, imageSource: CGImageSourceRef) -> Double {
    ...
    var dictionary = cfFrameProperties as NSDictionary
    if let gifProperties: NSDictionary = dictionary[kCGImagePropertyGIFDictionary as String] as? NSDictionary {
        if let delayTimeUnclampedProp: AnyObject = gifProperties[kCGImagePropertyGIFUnclampedDelayTime as String]? {
            frameDuration = delayTimeUnclampedProp as Double
        }
        else {
            if let delayTimeProp: AnyObject = gifProperties[kCGImagePropertyGIFDelayTime as String]? {
